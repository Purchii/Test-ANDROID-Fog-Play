...
from automation.network_offline_safe_runner.generate_network_offline_report import build_report, main
...
def _complete_metadata():
    return {
        ...
        "network_profile_policy": {
            "present": True,
            "evidence_status": "confirmed",
            "note": "Approved network profile policy placeholder for unit test.",
        },
        "resource_budget": {
            "present": True,
            "evidence_status": "confirmed",
            "note": "Approved resource budget placeholder for unit test.",
        },
        "redaction_policy": {
            "present": True,
            "evidence_status": "confirmed",
...
def test_missing_metadata_generates_blocked_network_offline_report():
    report = build_report()

    assert report["task_id"] == "TASK-007"
    ...
    assert report["prerequisites"]["network_profile_policy"]["present"] is False
...
def test_malformed_metadata_generates_blocked_report(tmp_path):
    metadata_path = tmp_path / "network_metadata.json"
...
def test_partial_or_non_confirmed_metadata_stays_blocked(tmp_path):
    ...
    metadata.pop("network_profile_policy")
    metadata["resource_budget"]["present"] = False
    ...
    assert report["overall_status"] == "blocked"
    assert "network_profile_policy is not approved or not present." in report["blocked_reasons"]
    assert "resource_budget is not approved or not present." in report["blocked_reasons"]
...
def test_invalid_evidence_status_normalizes_to_unknown_and_blocks(tmp_path):
    ...
    assert all(item["evidence_status"] == "unknown" for item in report["planned_network_offline_checks"])
    assert all(item["result"] != "pass" for item in report["planned_network_offline_checks"])
...
def test_complete_confirmed_metadata_generates_not_run_plan_without_pass(tmp_path):
    ...
    expected_categories = {
        "offline_startup",
        "offline_recovery",
        "reconnect_during_stream_guard",
        "high_latency_guard",
        "interruption_error_state",
        "redacted_network_evidence",
    }
    categories = {item["category"] for item in report["planned_network_offline_checks"]}

    assert report["overall_status"] == "not_run"
    assert report["evidence_status"] == "unknown"
    assert report["blocked_reasons"] == []
    assert categories == expected_categories
    assert all(item["result"] == "not_run" for item in report["planned_network_offline_checks"])
    assert all(item["evidence_status"] == "unknown" for item in report["planned_network_offline_checks"])
    assert '"pass"' not in json.dumps(report)


def test_cli_prints_network_offline_report_to_stdout(capsys):
    exit_code = main([])

    assert exit_code == 0
    report = json.loads(capsys.readouterr().out)
    assert report["overall_status"] == "blocked"
    assert report["planned_network_offline_checks"]


def test_cli_writes_network_offline_report_to_output_path(tmp_path):
    output_path = tmp_path / "network_offline_report.json"
...
def test_metadata_notes_and_artifacts_are_redacted(tmp_path):
    ...
    metadata["artifacts"] = [
        {
            "name": "network raw reference",
            "reference": f"{private_unix_path} {session_pair} {cookie_pair}",

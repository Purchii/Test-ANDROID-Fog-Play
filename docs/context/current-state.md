...
- TASK-015A/016 is active in thread `TASK-015A/016 - Approval validator hardening and ADB device/build inventory preflight` on branch `qa/task-015a-016-approval-validator-adb-inventory-preflight`. The task hardens TASK-015 approval validation against audit false-pass cases and adds TASK-016 inventory-only ADB preflight. Owner-approved local ADB inventory ran with raw outputs under ignored `.qa_local/devices/`; the final run saw no authorized ADB devices, so device collection is blocked while APK install, app launch and runtime smoke remain `not_run`.

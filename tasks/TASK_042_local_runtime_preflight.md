...
```text
qa/task-042-local-runtime-preflight
```
...
Existing local Android SDK/ADB/AVD; five APK bundle; launcher contour; device aliases

## Цель

Подтвердить, какие локальные runtime lanes реально готовы, не переизобретая пути и не ожидая внешних данных.
...
TASK-005 APK bundle contract; local paths policy; TASK-015A…017C device inventory validators; TASK-016 preflight history. use canonical .qa_local paths and aliases; no machine rediscovery

## In scope

- ровно пять основных APK по существующему bundle contract
- отдельный launcher/system contour по actual local checkout contract
- SDK tools, AVD list, ADB authorization and alias readiness
- local-only package/version/signature/hash metadata
- optional non-product AVD install/launch/tooling smoke
- machine-readable readiness matrix and selected next lane

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task042_scenarios.csv
```

Каталог содержит `18` сценариев, из них `15` P0. Основные
opaque surfaces: `SURF-INSTALL-001, SURF-RELEASE-001, SURF-LAUNCHER-001, SURF-START-001, SURF-COMPAT-001, SURF-PAIR-001, SURF-LOG-001, SURF-RELEASE-002`.
...
```text
automation/runtime_preflight/task042_local_runtime_preflight.py
tests/test_task042_local_runtime_preflight.py
...
- local-only raw preflight evidence
- public-safe APK/AVD/device readiness summary
- updated alias/status matrix
- selected control lane for TASK-044 and paired phone lane for TASK-045
- explicit FogPlay Stick availability status without guessing

## Out of scope

- global disk search while canonical paths are valid
- physical deep regression
- product compatibility claims from emulator
- production build or dependency acquisition
...
- five expected APK entries are classified; missing/extra entries do not false-pass
- launcher contour is separate from the five-APK bundle
- ADB/AVD/device states use canonical statuses and local-only identities
- tv-tpv-013, phone-xiaomi-007, tv-yandex-012, stb-sberdevices-009 and actual Stick alias are explicitly classified
- AVD result is labelled tooling-only
- no raw machine-specific value is committed
...
thread для `TASK-043`. Завершённый thread не реализует следующую независимую
задачу.

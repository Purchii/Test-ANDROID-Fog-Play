# TASK-054 — Crash, ANR, startup, resource, performance and soak qualification
...
```text
qa/task-054-stability-performance-soak
```

## Зависимости

TASK-044, TASK-045, TASK-049, TASK-051, TASK-052, TASK-053

## Основные lanes

Reference TV, paired phone, oldest TV OS, Sber STB and available system lane

## Цель

Получить QA-grade baseline стабильности, startup responsiveness, repeated lifecycle/reconnect behavior and bounded soak without production load.
...
all fresh representative runtime reports; crash/ANR summaries; TASK-051 reconnect and TASK-052 input cycles. bounded budgets and kill switches; no backend load

## In scope

- cold/warm launch timing and ready-state oracle
- focus/action response timing at user level
- repeated launch/background/force-stop/reconnect cycles
- bounded navigation and pairing soak
- memory/resource trend observations using standard Android tools
- crash/ANR/tombstone category capture and redaction
- thermal/battery observations where meaningful

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task054_scenarios.csv
```

Каталог содержит `18` сценариев, из них `14` P0. Основные
opaque surfaces: `SURF-PERF-001, SURF-START-001, SURF-START-002, SURF-FOCUS-001, SURF-PERF-002, SURF-NET-002, SURF-LOG-001, SURF-RELEASE-001`.
...
```text
automation/performance/task054_stability_performance_soak.py
tests/test_task054_stability_performance_soak.py
docs/qa/reports/task054_stability_performance_soak.summary.json
```
...
- performance/stability profile schema and runner
- baseline metrics with environment metadata
- cycle/soak ledger and anomaly thresholds
- crash/ANR/resource trend report
- release-impact classification

## Out of scope

- backend load/performance testing
- microbenchmark/source profiling
- unbounded overnight stress without budget/kill switch
...
- ready-state timing is based on actionable UI, not process start alone
- budgets, iteration counts and kill switch are explicit
- resource trends do not claim leaks without sufficient evidence
- every crash/ANR has local evidence and public-safe category summary
- soak failure/recovery is not collapsed into pass
...
git diff --check
python -m pytest -q tests/test_task054_stability_performance_soak.py
...
thread для `TASK-055`. Завершённый thread не реализует следующую независимую
задачу.

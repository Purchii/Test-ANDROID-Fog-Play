...
TASK-016A may run only inventory commands through
...
1. TASK-016A collects raw local-only ADB inventory when devices are visible.
2. TASK-016A generates public-safe inventory with heuristic classification.
...
No TASK-016A output confirms app launch, first visible state, focus behavior,

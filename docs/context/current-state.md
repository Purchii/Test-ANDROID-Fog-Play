...
- TASK-015E/017 is in progress in thread `TASK-015E/017 - Final approval metadata hardening + public-safe device inventory review package` on branch `qa/task-015e-017-final-metadata-inventory-review` from `main` commit `07018c2`. Phase A hardens exact local path families, synthetic QA user sub-policy, evidence retention, cleanup semantics and full-tree hygiene scanning. Phase B uses existing sanitized `.qa_local/devices/device_inventory.public_safe.generated.json` and exports public-safe owner-review inventory only; generated devices remain heuristic/manual-review-required and not approved for TASK-005.

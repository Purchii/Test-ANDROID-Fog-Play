...
| TASK-027 | Full app transition graph physical runtime coverage | NON_AUTONOMOUS runtime task after refreshed TASK-027 preflight and reviewer approvals | qa/task-027-full-app-transition-graph-physical-runtime | TASK-027R partial physical runtime executed; selected graph branches covered/classified with local-only evidence, anomaly ledger updated, and continuation required for unclosed rail-route branches/full graph closure |

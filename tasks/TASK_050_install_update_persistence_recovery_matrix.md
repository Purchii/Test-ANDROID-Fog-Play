# TASK-050 — Install, update, persistence, process-death and recovery matrix
...
```text
qa/task-050-install-update-persistence-recovery-matrix
```
...
Representative TV, phone and AOSP/launcher lanes where available

## Цель

Проверить установку, обновление между уже имеющимися локальными bundles, local/session state, process death, reboot и rollback-safe recovery.
...
TASK-005 install smoke; TASK-019 session persistence; TASK-024/025 native regression; TASK-049 graph. stateful before/action/after/cleanup evidence

## In scope

- clean install, reinstall, same-version install, incompatible/stale package cases
- update only when prior local approved bundle exists
- clear data, force-stop, kill process, reboot and foreground recovery
- auth/session/preferences/cache retention and reset semantics
- backup/restore observable behavior where safely available
- cross-app paired state cleanup

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task050_scenarios.csv
```

Каталог содержит `20` сценариев, из них `16` P0. Основные
opaque surfaces: `SURF-INSTALL-001, SURF-INSTALL-002, SURF-PERSIST-001, SURF-PERSIST-002, SURF-NET-003, SURF-PAIR-002, SURF-LAUNCHER-004, SURF-LOG-001, SURF-RELEASE-001`.
...
```text
automation/lifecycle/task050_install_persistence_recovery.py
tests/test_task050_install_persistence_recovery.py
docs/qa/reports/task050_install_persistence_recovery.summary.json
```
...
- stateful lifecycle runner and matrix
- before/after state ledger
- session/cache/persistence anomaly report
- cleanup/rollback verification
- family-specific install/update regression subset

## Out of scope

- obtaining older APKs externally
- downgrade forcing, signature bypass or APK resigning
- destructive production data manipulation
...
- missing prior local bundle is blocked_by_fixture, not pass
- each state-changing scenario has pre-state, action, post-state and cleanup evidence
- process death differs from force-stop/reboot in reporting
- unexpected data retention/loss is reported without guessing intended contract
- no signature/downgrade bypass is attempted
...
git diff --check
python -m pytest -q tests/test_task050_install_persistence_recovery.py
...
thread для `TASK-051`. Завершённый thread не реализует следующую независимую
задачу.

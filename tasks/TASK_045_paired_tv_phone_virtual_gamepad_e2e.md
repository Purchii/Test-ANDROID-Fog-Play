# TASK-045 — Paired Television Full plus Phone Full virtual-gamepad E2E
...
```text
qa/task-045-paired-tv-phone-virtual-gamepad-e2e
```
...
tv-tpv-013 + phone-xiaomi-007; phone-samsung-002 fallback

## Цель

Закрыть ключевой парный пользовательский путь TV↔phone и жизненный цикл виртуального геймпада до безопасной границы оплаты/игровой сессии.
...
TASK-022 gamepad inventory; TASK-032 DataChannel/gamepad contracts; existing virtual-gamepad support instruction; TASK-044 reference oracle. build paired black-box lane; no paid gameplay assertion

## In scope

- same synthetic account/session preconditions
- TV connect-device screen and phone discovery/connection
- cold/warm launch, foreground/background, lock/unlock
- duplicate connect, wrong-account/no-TV negative states
- safe network loss/reconnect on phone and TV
- virtual gamepad UI, enablement, input boundary and disconnect cleanup
- paired evidence synchronization and cross-device timeline

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task045_scenarios.csv
```

Каталог содержит `22` сценариев, из них `20` P0. Основные
opaque surfaces: `SURF-PAIR-001, SURF-AUTH-003, SURF-PAIR-002, SURF-INPUT-003, SURF-INPUT-004, SURF-PERSIST-001, SURF-PERSIST-002, SURF-STREAM-002, SURF-NET-002, SURF-BOUNDARY-001, SURF-STREAM-001, SURF-LOG-001, SURF-RELEASE-001`.
...
```text
automation/gamepad/task045_paired_virtual_gamepad.py
tests/test_task045_paired_virtual_gamepad.py
docs/qa/reports/task045_paired_virtual_gamepad.summary.json
```
...
- paired runner/orchestrator with rollback
- cross-device scenario and event ledger
- virtual-gamepad screen/state inventory
- reconnect/cleanup/anomaly reports
- reusable paired fixture contract

## Out of scope

- real payment or paid session start
- latency certification requiring active gameplay
- production account mutation
- physical gamepad coverage beyond paired-path comparison
...
- same-account success and wrong-account/no-TV negative behavior are distinguished
- phone restart/background/network recovery are independently evidenced
- duplicate connections or stale TV targets do not false-pass
- disconnect cleans up visible paired state
- TV and phone evidence is time-correlated without publishing raw identities
- boundary prevents real payment/session start
...
thread для `TASK-046`. Завершённый thread не реализует следующую независимую
задачу.

...
actual local FogPlay Stick alias / aosp-full + launcher/setup/services contour
...
TASK-002 exported component guards; TASK-005 local artifact policy; source-informed launcher risk IDs. actual compatible target only; no generic TV substitution

## In scope

- cluster inventory and compatibility preflight
- reboot/direct boot/HOME/launcher foreground behavior
- launcher windows, focus and application launch/return
- setup wizard safe states and recovery
- service/provider/receiver observable safe behavior
- process death/service restart/system overlays
- aosp-full app path and launcher integration
...
Каталог содержит `19` сценариев, из них `15` P0. Основные
opaque surfaces: `SURF-LAUNCHER-001, SURF-INSTALL-001, SURF-LAUNCHER-002, SURF-LAUNCHER-003, SURF-FOCUS-001, SURF-NAV-002, SURF-LAUNCHER-004, SURF-PERSIST-002, SURF-IPC-001, SURF-IPC-002, SURF-LOG-001, SURF-PERF-001, SURF-RELEASE-001`.
...
```text
automation/system_lane/task048_aosp_launcher_runtime.py
tests/test_task048_aosp_launcher_runtime.py
docs/qa/reports/task048_aosp_launcher_runtime.summary.json
```
...
- system-lane preflight and runtime runner
- launcher/window/setup/service state ledger
- AOSP app transition evidence
- blocked-by-device path that does not stop later tasks
- launcher-specific regression subset

## Out of scope

- generic Android TV as proof
- root/privilege escalation or system image modification
- unauthorized exported-component probing
...
thread для `TASK-049`. Завершённый thread не реализует следующую независимую
задачу.

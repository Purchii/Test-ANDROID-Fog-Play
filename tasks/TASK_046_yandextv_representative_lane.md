# TASK-046 — Television Steam / YandexTV representative runtime lane
...
```text
qa/task-046-yandextv-representative-lane
```
...
tv-yandex-012 / television-steam / Android 9 API 28

## Цель

Перенести контрольные TV oracles на Yandex-specific APK и нижнюю доступную TV OS-границу, фиксируя flavor/OS отличия.
...
TASK-044 reference oracle; compatibility matrix/report format TASK-009. execute family-specific evidence; reference is comparator only

## In scope

- install/start/auth/session/catalog/search/settings/detail boundary
- D-pad/focus/Back/Home/process lifecycle
- display and system keyboard behavior
- delta comparison with TPV13 reference lane
- flavor-specific observable integrations within safe boundary

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task046_scenarios.csv
```

Каталог содержит `17` сценариев, из них `15` P0. Основные
opaque surfaces: `SURF-INSTALL-001, SURF-START-001, SURF-AUTH-001, SURF-CATALOG-001, SURF-SEARCH-001, SURF-SEARCH-002, SURF-SETTINGS-001, SURF-SETTINGS-002, SURF-DETAIL-001, SURF-DETAIL-002, SURF-BOUNDARY-001, SURF-NAV-001, SURF-NAV-002, SURF-PERSIST-002, SURF-FOCUS-001, SURF-FOCUS-002, SURF-COMPAT-002, SURF-COMPAT-001, SURF-USABILITY-001, SURF-PERF-001, SURF-LOG-001, SURF-RELEASE-001`.
...
```text
automation/native_regression/task046_yandextv_representative_lane.py
tests/test_task046_yandextv_representative_lane.py
docs/qa/reports/task046_yandextv_representative_lane.summary.json
```
...
- Yandex representative runner and report
- reference-delta ledger
- OS/flavor anomalies and compatibility evidence
- selected regression subset for television-steam

## Out of scope

- assumption of equivalence with television-full
- real payment/session
- vendor account mutation or external service traversal
...
- all P0 reference scenarios are executed or explicitly classified
- API 28-specific behavior is separated from flavor behavior where evidence permits
- focus/system keyboard differences are recorded
- no TPV evidence is reused as Yandex pass
- crash/ANR and evidence integrity gates pass or block release
...
git diff --check
python -m pytest -q tests/test_task046_yandextv_representative_lane.py
...
thread для `TASK-047`. Завершённый thread не реализует следующую независимую
задачу.

...
| TASK-015C/016B | Approval/device-inventory consistency polish and local ADB inventory readiness | NON_AUTONOMOUS; validator/docs/tests are PROD_SAFE; local ADB inventory is owner-approved PROD_CONDITIONAL only when `adb` and devices are available | qa/task-015c-016b-approval-inventory-consistency | completed; default push authorized to detected `main` |

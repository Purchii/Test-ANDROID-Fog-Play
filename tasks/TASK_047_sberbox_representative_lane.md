# TASK-047 — Television Sber / SberBox representative runtime lane
...
```text
qa/task-047-sberbox-representative-lane
```
...
stb-sberdevices-009 / television-sber / Android 13 API 33

## Цель

Закрыть representative Sber flavor на STB form factor и выделить отличия от reference TV и будущей SberTV equivalence lane.
...
TASK-044 reference oracle; compatibility matrix/report format TASK-009. execute Sber-specific evidence; reference is comparator only

## In scope

- install/start/auth/session/catalog/search/settings/detail boundary
- D-pad/focus/Back/Home/process lifecycle
- STB display/input/system integration behavior
- delta comparison with TPV13
- Sber-specific observable integration boundaries

## Обязательный сценарный каталог

```text
docs/qa/epics/scenarios/task047_scenarios.csv
```
...
```text
automation/native_regression/task047_sberbox_representative_lane.py
tests/test_task047_sberbox_representative_lane.py
docs/qa/reports/task047_sberbox_representative_lane.summary.json
```
...
- SberBox representative runner/report
- STB and flavor delta ledger
- selected regression subset for television-sber
- compatibility handoff for tv-salutetv-011

## Out of scope

- SberTV equivalence proof
- real payment/session
- vendor account mutation or privileged bypass
...
- all P0 representative cases are executed or classified
- STB-specific focus/display/system behavior is captured
- no TPV/Yandex evidence substitutes Sber evidence
- vendor boundary is preserved
...
git diff --check
python -m pytest -q tests/test_task047_sberbox_representative_lane.py
...
thread для `TASK-048`. Завершённый thread не реализует следующую независимую
задачу.
